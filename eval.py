...
def analyze_memory_usage(rag_results, cfg: DictConfig, corpus_file: str, vis_image_file: str, 
    ...
    if root_dir and corpus_tag:
        ...
        try:
            ...
            print(f"[BEMR] (TextGrad ) to: {stats_after_file}")
...
def main(cfg: DictConfig):
    ...
    print(f"mode: {cfg.parameters.mode} | {cfg.model.source} | dataset: {cfg.experiment.test_dataset_name}")
    ...
    if cfg.parameters.mode in ['rag', 'all']:
        ...
        if is_code_task:
            ...
            acc_rag,_ = evaluate_results(rag_results, f"SleepRAG ({corpus_tag} Memory)", result_log_file)
